...
class FinancialModelAgent:
    ...
    def __init__(self):
        ...
        self.description = "Consolidates cost and value projections, calculates financial metrics."
        self.status = "initialized"
        
        print(f"FinancialModelAgent: Initialized successfully.")
        self.status = "available"

    async def generate_financial_summary(self, cost_estimate: Dict[str, Any], value_projection: Dict[str, Any], case_title: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if cost_currency != value_currency:
                logger.warning(f"[FinancialModelAgent] Currency mismatch: Cost ({cost_currency}) vs Value ({value_currency})")
                # For V1, we'll continue with a warning but use the cost currency as primary
            
            primary_currency = cost_currency
            
            # Calculate basic financial metrics
            financial_metrics = self._calculate_financial_metrics(total_cost, value_scenarios, primary_currency)
            ...
            summary_data = {
                ...
                "cost_breakdown_source": cost_estimate.get('rate_card_used', 'Default rates'),
                "value_methodology": value_projection.get('methodology', 'Standard projection'),
                "notes": "Initial financial summary based on approved estimates.",
                "generated_timestamp": None  # Will be set by orchestrator
            }
            
            logger.info(f"[FinancialModelAgent] Successfully generated financial summary for {case_title}")
            return {
                "status": "success", 
    ...
    def _extract_total_cost(self, cost_estimate: Dict[str, Any]) -> float:
        ...
        if estimated_cost < 0:
            raise ValueError(f"Invalid estimated_cost value: {estimated_cost} (cannot be negative)")
        
        return float(estimated_cost)

    def _extract_value_scenarios(self, value_projection: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        for scenario in scenarios:
            ...
            if value is None:
                logger.warning(f"[FinancialModelAgent] Missing value for scenario '{case_name}', skipping")
                continue
            
            if not isinstance(value, (int, float)):
                logger.warning(f"[FinancialModelAgent] Invalid value type for scenario '{case_name}': {type(value)}, skipping")
                continue
    ...
    def _calculate_financial_metrics(self, total_cost: float, value_scenarios: Dict[str, float], currency: str) -> Dict[str, Any]:
        """
        ...
        for scenario_name, scenario_value in value_scenarios.items():
            ...
            if total_cost > 0:
                ...
                logger.warning("[FinancialModelAgent] Cannot calculate ROI: total cost is zero")
        ...
        if not base_scenario_value:
            ...
            logger.info(f"[FinancialModelAgent] No 'Base' scenario found, using '{base_scenario_name}' for primary metrics")
        ...
        if base_scenario_value > 0:
            ...
            metrics["payback_period_note"] = "Simplified calculation assuming annual benefits equal to projected value"
        else:
            metrics["simple_payback_period_years"] = "N/A"
            metrics["payback_period_note"] = "Cannot calculate: zero or negative projected value"
