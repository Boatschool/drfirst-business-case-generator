...
class PlannerAgent:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"PlannerAgent: Vertex AI initialized successfully with model {self.model_name}.")
    ...
    async def estimate_effort(self, prd_content: str, system_design_content: str, case_title: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.model:
                effort_data = await self._ai_effort_estimation(prd_content, system_design_content, case_title)
                if effort_data:
                    logger.info(f"[PlannerAgent] AI-powered effort estimation completed for {case_title}: {effort_data['total_hours']} total hours")
                    print(f"[PlannerAgent] Generated AI-powered effort estimate: {effort_data['total_hours']} total hours across {len(effort_data['roles'])} roles")
            ...
            print(f"[PlannerAgent] Falling back to keyword-based estimation for {case_title}")
            effort_data = await self._keyword_effort_estimation(prd_content, system_design_content, case_title)
            
            logger.info(f"[PlannerAgent] Keyword-based effort estimation completed for {case_title}: {effort_data['total_hours']} total hours")
            print(f"[PlannerAgent] Generated keyword-based effort estimate: {effort_data['total_hours']} total hours across {len(effort_data['roles'])} roles")
    ...
    async def _ai_effort_estimation(self, prd_content: str, system_design_content: str, case_title: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            truncated_prd = prd_content[:max_content_length] if prd_content else "No PRD content provided"
            truncated_system_design = system_design_content[:max_content_length] if system_design_content else "No System Design content provided"
            
            prompt = f"""You are a Senior Project Planner with expertise in healthcare technology projects. Based on the following PRD and System Design for a project titled '{case_title}', estimate the development effort in hours for each role.
...
Make sure your response is valid JSON without any additional text or markdown formatting."""
            ...
            print(f"[PlannerAgent] Calling AI model for effort estimation...")
            ...
            if response.candidates and response.candidates[0].content.parts:
                ...
                try:
                    ...
                    if json_match:
                        ...
                        if self._validate_effort_data(effort_data):
                            print(f"[PlannerAgent] Successfully parsed AI effort estimation")
                            return effort_data
                        else:
                            print(f"[PlannerAgent] AI response validation failed")
                            return None
                    else:
                        print(f"[PlannerAgent] No valid JSON found in AI response")
                        return None
                ...
                print(f"[PlannerAgent] No valid response from AI model")
                return None
    ...
    async def _keyword_effort_estimation(self, prd_content: str, system_design_content: str, case_title: str) -> Dict[str, Any]:
        """
        ...
        complexity_indicators = {
            ...
            'machine learning': 80, 'ai integration': 80, 'natural language': 60,
            'real-time': 50, 'microservices': 60, 'distributed': 50,
            'blockchain': 100, 'encryption': 40, 'hipaa': 30,
            
            # Medium complexity features  
            'api integration': 30, 'database': 25, 'authentication': 25,
            'mobile app': 40, 'web application': 30, 'dashboard': 25,
            'reporting': 30, 'notification': 20, 'search': 25,
            
            # Basic features
            'crud operations': 15, 'form': 10, 'validation': 10,
            'user management': 20, 'admin panel': 25, 'email': 15,
            
            # Integration complexity
            'third party': 35, 'external api': 35, 'hl7': 40,
            'fhir': 40, 'ehr integration': 60, 'payment': 45,
            
            # Technical complexity
            'scalability': 30, 'performance': 25, 'caching': 20,
            'load balancing': 35, 'monitoring': 25, 'logging': 15,
        }
        ...
        notes = f"Keyword-based estimation (complexity score: {total_complexity_score}). "
        if found_keywords:
    ...
    def _validate_effort_data(self, data: Dict[str, Any]) -> bool:
        ...
        """
        required_fields = ["roles", "total_hours", "estimated_duration_weeks", "complexity_assessment", "notes"]
        ...
        if not isinstance(data["roles"], list) or len(data["roles"]) == 0:
            print(f"[PlannerAgent] Invalid roles structure")
            return False
        
        for role_data in data["roles"]:
            if not isinstance(role_data, dict) or "role" not in role_data or "hours" not in role_data:
                print(f"[PlannerAgent] Invalid role data structure")
                return False
            if not isinstance(role_data["hours"], (int, float)) or role_data["hours"] < 0:
                print(f"[PlannerAgent] Invalid hours value")
                return False
        
        # Check other fields
        if not isinstance(data["total_hours"], (int, float)) or data["total_hours"] < 0:
            print(f"[PlannerAgent] Invalid total_hours")
            return False
        
        if not isinstance(data["estimated_duration_weeks"], (int, float)) or data["estimated_duration_weeks"] < 0:
            print(f"[PlannerAgent] Invalid estimated_duration_weeks")
            return False
            
        valid_complexity = ["Low", "Medium", "High", "Very High"]
        if data["complexity_assessment"] not in valid_complexity:
            print(f"[PlannerAgent] Invalid complexity_assessment")
            return False
