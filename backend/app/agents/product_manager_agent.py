...
class ProductManagerAgent:
    ...
    async def summarize_content(self, text_content: str, link_name: str = "content") -> str:
        """
        ...
        try:
            ...
            summarization_prompt = f"""Please analyze the following text content and provide a concise summary focusing on information relevant to a software/technology project or business case:
...
Summary:"""
            ...
            logger.info(f"[ProductManagerAgent] Generating summary for content from: {link_name}")
            ...
            if response.candidates and response.candidates[0].content.parts:
                ...
                logger.info(f"[ProductManagerAgent] Successfully generated summary for {link_name}")
                return summary
            else:
                logger.warning(f"[ProductManagerAgent] No summary generated for {link_name}")
                return ""
                
        except Exception as e:
            logger.error(f"[ProductManagerAgent] Error generating summary for {link_name}: {str(e)}")
            return ""
    ...
    async def draft_prd(self, problem_statement: str, case_title: str, relevant_links: List[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        links_context = ""
        if relevant_links:
            print(f"[ProductManagerAgent] Processing {len(relevant_links)} relevant links for context...")
            ...
            for link_item in relevant_links:
                ...
                print(f"[ProductManagerAgent] Fetching content from: {link_name} ({link_url})")
                ...
                try:
                    ...
                    if web_result["success"] and web_result["content"]:
                        ...
                        summary = await self.summarize_content(web_result["content"], link_name)
                        
                        if summary and summary.strip():
                            links_context += f"- **{link_name}** ({link_url}):\n"
                            links_context += f"  Content Summary: {summary}\n\n"
                            print(f"[ProductManagerAgent] Successfully processed content from {link_name}")
                        else:
                            links_context += f"- **{link_name}** ({link_url}): Content retrieved but no relevant summary generated\n\n"
                            print(f"[ProductManagerAgent] Content retrieved from {link_name} but summarization failed or not relevant")
                    else:
                        error_msg = web_result.get("error", "Unknown error")
                        links_context += f"- **{link_name}** ({link_url}): Unable to fetch content - {error_msg}\n\n"
                        print(f"[ProductManagerAgent] Failed to fetch content from {link_name}: {error_msg}")
                        
                except Exception as e:
                    error_msg = f"Unexpected error: {str(e)}"
                    links_context += f"- **{link_name}** ({link_url}): {error_msg}\n\n"
                    print(f"[ProductManagerAgent] Unexpected error processing {link_name}: {str(e)}")
            
            if links_context.strip() == "Additional Context from Relevant Links:":
                # No successful content was retrieved
                links_context += "No content could be retrieved from the provided links.\n"
            else:
        ...
        if configurable_prompt:
            ...
            print(f"[ProductManagerAgent] Using configurable prompt from Firestore")
        else:
            print(f"[ProductManagerAgent] No configurable prompt found, using fallback default prompt")
