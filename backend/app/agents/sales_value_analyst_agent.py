...
class SalesValueAnalystAgent:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"SalesValueAnalystAgent: Vertex AI initialized successfully with model {self.model_name}.")
    ...
    async def _fetch_pricing_template(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info("[SalesValueAnalystAgent] Looking for active + default pricing templates")
            active_default_query = templates_ref.where("isActive", "==", True).where("isDefault", "==", True)
            active_default_docs = await asyncio.to_thread(lambda: list(active_default_query.limit(1).stream()))
            
            if active_default_docs:
                template_data = active_default_docs[0].to_dict()
                template_data['id'] = active_default_docs[0].id
                logger.info(f"[SalesValueAnalystAgent] Found active + default template: {template_data.get('name', 'Unknown')}")
                return template_data
            
            # Strategy 2: Look for any active template
            logger.info("[SalesValueAnalystAgent] Looking for any active pricing template")
            active_query = templates_ref.where("isActive", "==", True)
            active_docs = await asyncio.to_thread(lambda: list(active_query.limit(1).stream()))
            
            if active_docs:
                template_data = active_docs[0].to_dict()
                template_data['id'] = active_docs[0].id
                logger.info(f"[SalesValueAnalystAgent] Found active template: {template_data.get('name', 'Unknown')}")
                return template_data
            
            # Strategy 3: Fall back to specific document lookup
            logger.info("[SalesValueAnalystAgent] Looking for specific default template document")
            ...
            if doc_snapshot.exists:
                ...
                logger.info(f"[SalesValueAnalystAgent] Found specific template: {template_data.get('name', 'Unknown')}")
                return template_data
            else:
                logger.warning("[SalesValueAnalystAgent] No pricing templates found in any strategy")
                return None
    ...
    async def _project_with_template(self, prd_content: str, template: Dict[str, Any], case_title: str) -> Dict[str, Any]:
        ...
        """
        logger.info(f"[SalesValueAnalystAgent] Using template '{template.get('name', 'Unknown')}' for value projection")
        
        # Try AI-powered generation first if available
        if self.vertex_ai_available and self.model:
            try:
                logger.info("[SalesValueAnalystAgent] Attempting AI-powered value projection")
                ai_result = await self._project_with_ai_template(prd_content, template, case_title)
                if ai_result and ai_result.get("status") == "success":
                    logger.info("[SalesValueAnalystAgent] AI-powered projection successful")
                    return ai_result
                else:
                    logger.warning("[SalesValueAnalystAgent] AI-powered projection failed, falling back to template-based")
            except Exception as e:
                logger.warning(f"[SalesValueAnalystAgent] AI generation error: {str(e)}, falling back to template-based")
        
        # Fall back to template-based generation
        return await self._project_with_template_fallback(prd_content, template, case_title)

    async def _project_with_ai_template(self, prd_content: str, template: Dict[str, Any], case_title: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            prompt = f"""You are an experienced Sales/Value Analyst at DrFirst, a healthcare technology company. You are tasked with projecting realistic business value and revenue scenarios for a technology business case.
...
Generate the value projection now:"""
            ...
            if response.candidates and response.candidates[0].content.parts:
                ...
                logger.info(f"[SalesValueAnalystAgent] Received AI response ({len(ai_response_text)} characters)")
                
                # Parse AI response into structured format
                value_data = await self._parse_ai_response(ai_response_text, template)
                
                if value_data:
                    logger.info("[SalesValueAnalystAgent] Successfully parsed AI response")
                    return {
                        "status": "success",
                    ...
                    logger.warning("[SalesValueAnalystAgent] Failed to parse AI response")
    ...
    def _extract_prd_summary(self, prd_content: str) -> str:
        ...
        for line in lines:
            ...
            if line.startswith('#') and any(keyword in line.lower() for keyword in 
                ['problem', 'goal', 'objective', 'solution', 'feature', 'user']):
    ...
    def _format_template_guidance(self, guidance: Dict[str, Any], structure: Dict[str, Any]) -> str:
        """
        ...
        return guidance_text if guidance_text else "No specific guidance provided in template."

    async def _parse_ai_response(self, ai_text: str, template: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
            ...
            if json_match:
                ...
                if "scenarios" in parsed_data and isinstance(parsed_data["scenarios"], list):
                    ...
                    if "methodology" not in parsed_data:
                        parsed_data["methodology"] = "AI-powered value projection with template guidance"
                    if "assumptions" not in parsed_data:
                        parsed_data["assumptions"] = ["AI-generated assumptions based on PRD analysis"]
                    
                    logger.info(f"[SalesValueAnalystAgent] Successfully parsed JSON with {len(parsed_data['scenarios'])} scenarios")
                    return parsed_data
            
            # Fallback: manual extraction
            logger.info("[SalesValueAnalystAgent] JSON parsing failed, attempting manual extraction")
            return self._manual_extract_scenarios(ai_text, template)
            
        except json.JSONDecodeError as e:
            logger.warning(f"[SalesValueAnalystAgent] JSON parsing error: {e}, attempting manual extraction")
            return self._manual_extract_scenarios(ai_text, template)
        except Exception as e:
            logger.error(f"[SalesValueAnalystAgent] Error parsing AI response: {e}")
            return None

    def _manual_extract_scenarios(self, ai_text: str, template: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if scenarios:
                return {
                    ...
                    "assumptions": ["Extracted from AI analysis", "Healthcare technology value factors"],
    ...
    async def _project_with_template_fallback(self, prd_content: str, template: Dict[str, Any], case_title: str) -> Dict[str, Any]:
        """
        ...
        if template_type == "LowBaseHigh":
            ...
            if predefined_scenarios and len(predefined_scenarios) >= 3:
                ...
                for scenario in predefined_scenarios[:3]:  # Take first 3
                    scenarios.append({
                        ...
                        "description": scenario.get("description", "Template-defined scenario")
                    })
            else:
                # Use default scenarios with template context
                scenarios = [
                    {"case": "Low", "value": 5000, "description": "Conservative estimate based on minimal adoption and template guidance."},
                    {"case": "Base", "value": 15000, "description": "Most likely estimate based on expected adoption and template framework."},
                    {"case": "High", "value": 30000, "description": "Optimistic estimate based on high adoption and template opportunities."}
                ]
            ...
            value_data = {
                "scenarios": [
                    {"case": "Baseline", "value": 12000, "description": f"Standard value estimate using {template_type} methodology."}
                ],
                "currency": "USD",
                "template_used": template.get("name", "Unknown Template"),
        ...
        logger.info(f"[SalesValueAnalystAgent] Successfully projected value for {case_title} using template fallback")
