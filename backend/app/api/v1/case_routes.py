...
class BusinessCaseDetailsModel(BaseModel):
    ...
    effort_estimate_v1: Optional[Dict[str, Any]] = (
        None  # New: Effort estimate from PlannerAgent
    )
    cost_estimate_v1: Optional[Dict[str, Any]] = (
        None  # New: Cost estimate from CostAnalystAgent
    )
    value_projection_v1: Optional[Dict[str, Any]] = (
        None  # New: Value projection from SalesValueAnalystAgent
    )
    financial_summary_v1: Optional[Dict[str, Any]] = (
        None  # New: Financial summary from FinancialModelAgent
    )
    created_at: datetime
    updated_at: datetime
...
async def list_user_cases(
    ...
    try:
        ...
        docs_list.sort(
            key=lambda doc: doc.to_dict().get("updated_at", datetime.min), reverse=True
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve business cases: {str(e)}"
        )
...
async def get_case_details(
    ...
    try:
        ...
        if not data:  # Should not happen if doc.exists is true, but good practice
            raise HTTPException(
                status_code=404, detail=f"Business case {case_id} data is empty."
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve business case details: {str(e)}",
        )
...
async def update_prd_draft(
    ...
    try:
        ...
        updated_prd_draft = {
            "title": case_data.get("title", "PRD")
            + " - Draft",  # Or derive title differently
            "content_markdown": prd_update_request.content_markdown,
            "version": existing_prd_draft.get(
                "version", "1.0.0"
            ),  # Basic versioning, could be incremented
...
async def submit_prd_for_review(
    ...
    try:
        ...
        error_details = traceback.format_exc()
        print(
            f"Error submitting PRD for review for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
    ...
    try:
        ...
        try:
            ...
            if design_result.get("status") == "success":
                ...
                print(
                    f"System design generation failed for case {case_id}: {design_result.get('message')}"
                )
                return {
            ...
            print(
                f"Error initiating system design for case {case_id}: {str(design_error)}"
            )
            return {
...
async def update_case_status(
    ...
    if status_update_request.status not in VALID_STATUSES:
        raise HTTPException(
            status_code=400, detail=f"Invalid status: {status_update_request.status}"
        )
    ...
    try:
        ...
        if case_data.get("user_id") != user_id:
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to update this business case status.",
            )
...
@router.put(
    ...
    status_code=200,
    summary="Update System Design for a specific business case",
)
async def update_system_design_draft(
    ...
    try:
        ...
        updated_system_design = {
            ...
            "generated_by": existing_system_design.get(
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to update system design: {str(e)}"
        )
...
async def submit_system_design_for_review(
    ...
    try:
        ...
        if not (is_owner or is_developer):
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to submit this system design.",
            )
        ...
        if not case_data.get("system_design_v1_draft"):
            raise HTTPException(
                status_code=400, detail="No system design draft found to submit."
            )
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to submit system design: {str(e)}"
        )
...
async def approve_system_design(
    ...
    if system_role != "DEVELOPER":
        raise HTTPException(
            status_code=403,
            detail="Only users with DEVELOPER role can approve system designs.",
        )
    ...
    try:
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to approve system design: {str(e)}"
        )
...
async def reject_system_design(
    ...
    if system_role != "DEVELOPER":
        raise HTTPException(
            status_code=403,
            detail="Only users with DEVELOPER role can reject system designs.",
        )
    ...
    try:
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to reject system design: {str(e)}"
        )
...
async def update_effort_estimate(
    ...
    try:
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to update effort estimate: {str(e)}"
        )
...
async def submit_effort_estimate_for_review(
    ...
    try:
        ...
        if not case_data.get("effort_estimate_v1"):
            raise HTTPException(
                status_code=400, detail="No effort estimate found to submit."
            )
        ...
        error_details = traceback.format_exc()
        print(
            f"Error submitting effort estimate for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to submit effort estimate: {str(e)}"
        )


@router.put(
    "/cases/{case_id}/cost-estimate",
    status_code=200,
    summary="Update Cost Estimate for a specific business case",
)
async def update_cost_estimate(
    ...
    try:
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to update cost estimate: {str(e)}"
        )


@router.post(
    "/cases/{case_id}/cost-estimate/submit",
    status_code=200,
...
async def submit_cost_estimate_for_review(
    ...
    try:
        ...
        if not case_data.get("cost_estimate_v1"):
            raise HTTPException(
                status_code=400, detail="No cost estimate found to submit."
            )
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to submit cost estimate: {str(e)}"
        )
...
async def update_value_projection(
    ...
    try:
        ...
        error_details = traceback.format_exc()
        print(
            f"Error updating value projection for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to update value projection: {str(e)}"
        )


@router.post(
    "/cases/{case_id}/value-projection/submit",
    status_code=200,
...
async def submit_value_projection_for_review(
    ...
    try:
        ...
        if not case_data.get("value_projection_v1"):
            raise HTTPException(
                status_code=400, detail="No value projection found to submit."
            )
        ...
        error_details = traceback.format_exc()
        print(
            f"Error submitting value projection for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to submit value projection: {str(e)}"
        )
...
async def approve_effort_estimate(
    ...
    try:
        ...
        if case_data.get("user_id") != user_id:
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to approve this effort estimate.",
            )
        ...
        error_details = traceback.format_exc()
        print(
            f"Error approving effort estimate for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to approve effort estimate: {str(e)}"
        )
...
async def reject_effort_estimate(
    ...
    try:
        ...
        if case_data.get("user_id") != user_id:
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to reject this effort estimate.",
            )
        ...
        error_details = traceback.format_exc()
        print(
            f"Error rejecting effort estimate for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to reject effort estimate: {str(e)}"
        )
    ...
    try:
        ...
        try:
            ...
            financial_model_result = (
                await orchestrator.check_and_trigger_financial_model(case_id)
            )
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to approve cost estimate: {str(e)}"
        )
...
async def reject_cost_estimate(
    ...
    try:
        ...
        if case_data.get("user_id") != user_id:
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to reject this cost estimate.",
            )
        ...
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to reject cost estimate: {str(e)}"
        )
...
async def approve_value_projection(
    ...
    try:
        ...
        error_details = traceback.format_exc()
        print(
            f"Error approving value projection for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to approve value projection: {str(e)}"
        )
...
async def reject_value_projection(
    ...
    try:
        ...
        error_details = traceback.format_exc()
        print(
            f"Error rejecting value projection for case {case_id}, user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
            status_code=500, detail=f"Failed to reject value projection: {str(e)}"
        )
...
async def submit_case_for_final_approval(
    ...
    try:
        ...
        error_details = traceback.format_exc()
        print(
            f"Error submitting case {case_id} for final approval by user {user_id}: {e}"
        )
        print(f"Full traceback: {error_details}")
        raise HTTPException(
    ...
    try:
        ...
        if case_data.get("user_id") != user_id:
            ...
            if user_role not in ["ADMIN", "FINAL_APPROVER"]:
                raise HTTPException(
                    status_code=403,
                    detail="You do not have permission to export this business case.",
                )
