...
async def update_prompt(
    ...
    try:
        success = await prompt_service.update_prompt(prompt_id, update_data, current_user["uid"])
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Prompt not found"
...
async def add_prompt_version(
    ...
    try:
        success = await prompt_service.add_prompt_version(prompt_id, version_data, current_user["uid"])
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Prompt not found"
...
async def get_active_prompt_template(
    ...
    try:
        template = await prompt_service.get_active_prompt_template(agent_name, agent_function)
        if not template:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
async def render_prompt_template(
    ...
    try:
        rendered = await prompt_service.render_prompt(agent_name, agent_function, variables)
        if not rendered:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
