...
class AgentPrompt(BaseModel):
    ...
    agent_function: str = Field(
        ..., description="Specific function/task this prompt handles"
    )

    # Prompt template with placeholders
    current_version: str = Field(..., description="Currently active version identifier")
    versions: List[AgentPromptVersion] = Field(
        default_factory=list, description="All versions of this prompt"
    )
    ...
    category: str = Field(
        default="general",
        description="Category (e.g., 'prd_generation', 'system_design')",
    )
    ...
    ai_model_config: Dict[str, Any] = Field(
        ...
        description="Model-specific configuration (temperature, tokens, etc.)",
    )

    # Management
    is_enabled: bool = Field(default=True, description="Whether this prompt is enabled")
    created_at: datetime = Field(
        default_factory=datetime.now, description="When this prompt was created"
    )
    updated_at: datetime = Field(
        default_factory=datetime.now, description="When this prompt was last updated"
    )
    created_by: str = Field(..., description="User ID who created this prompt")
    last_updated_by: str = Field(
        ..., description="User ID who last updated this prompt"
    )

    # Usage tracking
    usage_count: int = Field(
        default=0, description="How many times this prompt has been used"
    )
    last_used_at: Optional[datetime] = Field(
        None, description="When this prompt was last used"
    )
