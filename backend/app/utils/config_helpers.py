...
async def get_final_approver_role_name() -> str:
    ...
    if (_final_approver_role_cache["role"] is not None and 
        _final_approver_role_cache["last_updated"] is not None and
        current_time - _final_approver_role_cache["last_updated"] < _final_approver_role_cache["cache_duration"]):
    ...
    try:
        ...
        if not doc.exists:
            ...
            print("[CONFIG] No final approver configuration found, using default 'FINAL_APPROVER'")
            final_approver_role = "FINAL_APPROVER"
        else:
            config_data = doc.to_dict()
            final_approver_role = config_data.get("finalApproverRoleName", "FINAL_APPROVER")
            print(f"[CONFIG] Retrieved final approver role from Firestore: {final_approver_role}")
...
def require_dynamic_final_approver_role():
    ...
    """
    async def dynamic_role_checker(current_user: dict = Depends(get_current_active_user)) -> dict:
        try:
            ...
            if user_role != required_role and user_role != 'ADMIN':
                print(f"[CONFIG] Access denied for {user_email} - requires '{required_role}' role, has: '{user_role}'")
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Access denied. Required role: {required_role} (or ADMIN)",
                )
            
            print(f"[CONFIG] Dynamic final approval access granted for {user_email} with role '{user_role}' (required: '{required_role}')")
            return current_user
