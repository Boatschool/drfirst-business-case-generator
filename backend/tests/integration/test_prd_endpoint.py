...
class TestPrdUpdateEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_successful_prd_update(
        self, mock_user, mock_case_data, prd_update_request
    ):
        ...
        with patch("app.api.v1.case_routes.firestore.Client") as mock_firestore:
            ...
            with patch("app.api.v1.case_routes.asyncio.to_thread") as mock_to_thread:
                ...
                assert (
                    result["updated_prd_draft"]["content_markdown"]
                    == prd_update_request.content_markdown
                )
                assert (
                    result["updated_prd_draft"]["title"] == "Test Business Case - Draft"
                )
    ...
    @pytest.mark.asyncio
    async def test_history_entry_creation(
        self, mock_user, mock_case_data, prd_update_request
    ):
