...
class TestRoleAssignmentScripts:
    ...
    def test_universal_script_validation(self):
        ...
        for role in UserRole:
            assert role.value in ROLE_DESCRIPTIONS, f"Missing description for role {role.value}"
        
        # Test description format
        for role_name, description in ROLE_DESCRIPTIONS.items():
            assert isinstance(description, str)
            assert len(description) > 0
            assert description.startswith(('🔑', '👤', '👁️', '👨‍💻', '💼', '📊', '💰', '⚖️', '🏗️', '📦', '📈'))
    ...
    async def test_role_assignment_validation(self):
        ...
        with patch('firebase_admin.auth.get_user_by_email') as mock_get_user:
            with patch.object(user_service, 'create_or_update_user') as mock_create_user:
    ...
    def test_specific_role_scripts_exist(self):
        ...
        for script_file in script_files:
            ...
            assert os.path.exists(script_path), f"Script file {script_file} should exist"
            assert os.path.isfile(script_path), f"{script_file} should be a file"
            
            # Check that file is executable (has shebang)
            with open(script_path, 'r') as f:
                first_line = f.readline().strip()
                assert first_line.startswith('#!'), f"{script_file} should start with shebang"
...
class TestRoleBasedFeatures:
    ...
    def test_admin_permissions(self):
        ...
        admin_accessible_endpoints = [
            "admin_panel", "user_management", "rate_cards", "pricing_templates",
            "business_case_approval", "system_design_approval", "financial_approval"
        ]
    ...
    def test_role_specific_permissions(self):
        ...
        permission_matrix = {
            ...
            UserRole.TECHNICAL_ARCHITECT: ["architecture_review", "system_design_approval"],
    ...
    def test_role_exclusions(self):
        ...
        sensitive_operations = ["user_role_assignment", "system_configuration", "admin_panel"]
        
        non_admin_roles = [
            UserRole.USER, UserRole.VIEWER, UserRole.SALES_REP,
            UserRole.BUSINESS_ANALYST, UserRole.DEVELOPER
        ]
...
class TestRoleWorkflowIntegration:
    ...
    def test_approval_workflow_roles(self):
        ...
        approval_workflows = {
            ...
            "system_design_approval": [UserRole.DEVELOPER, UserRole.TECHNICAL_ARCHITECT, UserRole.ADMIN],
        ...
        for workflow, approver_roles in approval_workflows.items():
            ...
            assert len(approver_roles) >= 1, f"Workflow {workflow} should have at least one approver"
            assert UserRole.ADMIN in approver_roles, f"Admin should be able to approve {workflow}"
