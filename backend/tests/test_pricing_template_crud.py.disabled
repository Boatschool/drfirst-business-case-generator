...
def test_update_pricing_template(template_id: str):
    ...
    try:
        response = requests.put(
            f"{ADMIN_ENDPOINTS}/{template_id}", json=update_data, timeout=10
        )
        ...
        if response.status_code == 401:
            ...
            print(
                f"   ✅ Template updated successfully: {updated_template.get('name')}"
            )
...
def test_delete_pricing_template(template_id: str):
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            print(
                f"   ✅ Template deleted successfully: {result.get('message', 'No message')}"
            )
        elif response.status_code == 404:
