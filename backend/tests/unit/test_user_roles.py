...
class TestUserRoleEnum:
    ...
    def test_all_roles_exist(self):
        ...
        assert actual_roles == expected_roles, f"Missing or extra roles. Expected: {expected_roles}, Got: {actual_roles}"
    ...
    def test_role_creation_from_string(self):
        ...
        valid_strings = [
            "ADMIN", "USER", "VIEWER", "DEVELOPER",
            "SALES_REP", "SALES_MANAGER", "FINANCE_APPROVER", 
            "LEGAL_APPROVER", "TECHNICAL_ARCHITECT", "PRODUCT_OWNER", "BUSINESS_ANALYST"
        ]
    ...
    def test_role_hierarchy_concepts(self):
        ...
        for role, level in privilege_levels.items():
            if role != UserRole.ADMIN:
                assert admin_level > level, f"Admin should have higher privilege than {role}"
        ...
        for approver in approver_roles:
            assert privilege_levels[approver] >= 5, f"{approver} should have high privilege level"
        
        # Test that viewer has lowest level
        viewer_level = privilege_levels[UserRole.VIEWER]
        for role, level in privilege_levels.items():
            if role != UserRole.VIEWER:
                assert viewer_level <= level, f"Viewer should have lowest or equal privilege to {role}"
